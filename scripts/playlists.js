...
export function PlaylistsUI(player) {
  ...
  const d = getData(); const pl = d.playlists[id]; if (!pl) return; currentId = id; detail.dataset.id = pl.id; plTitle.textContent = pl.name; plCount.textContent = `${pl.trackIds.length} bài hát`; detail.hidden = false; plTableBody.innerHTML = ''; let idx = 0; for (const tid of pl.trackIds) {
