...
export function PlaylistsUI(player) {
  ...
  function renderCards() {
    const d = getData();
    cardsC.innerHTML = '';
    const ids = Object.keys(d.playlists);
    if (!ids.length) {
      cardsC.innerHTML = `<div class="card"><div class="cover">🎶</div><div class="meta"><div><div class="title">Chưa có playlist</div><div class="muted">Tạo một playlist mới nhé!</div></div></div></div>`;
      return;
    }
    ids.forEach(id => {
      const pl = d.playlists[id];
      // Đảm bảo luôn có mảng id bài hát
  ...
  async function openDetail(id) {
    ...
    if (!pl) return;
    currentId = id;
    detail.dataset.id = pl.id;
    plTitle.textContent = pl.name;
    // Đảm bảo luôn có mảng id bài hát
    const trackIds = Array.isArray(pl.trackIds) ? pl.trackIds : (Array.isArray(pl.tracks) ? pl.tracks.map(t => (typeof t === 'string' ? t : t._id || t.id)) : []);
    plCount.textContent = `${trackIds.length} bài hát`;
    detail.hidden = false;
    plTableBody.innerHTML = '';
    let idx = 0;
    for (const tid of trackIds) {
      const tr = await idb.getTrack(tid);
      if (!tr) continue;
      const row = document.createElement('tr');
      row.draggable = true;
      row.dataset.id = tid;
      row.innerHTML = `<td>${++idx}</td><td>${tr.title}</td><td class="muted">${tr.artist || '—'}</td><td class="muted">${tr.duration ? formatTime(tr.duration) : '—'}</td><td class="actions"><button class="icon-btn sm remove" title="Xoá khỏi playlist"><img src="assets/icons/delete.svg" width="18" height="18"/></button></td>`;
      row.querySelector('.remove').onclick = () => removeFromCurrent(tid);
