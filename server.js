...
const app = express();
// Railway cấp PORT động, phải lấy từ biến môi trường
...
app.post('/api/tracks/upload', authenticateToken, upload.single('audio'), async (req, res) => {
  try {
    ...
    const cloudRes = await cloudinary.uploader.upload(req.file.path, {
      resource_type: 'video', // Cloudinary dùng 'video' cho file audio
      folder: 'flowplay-audio'
    });
    ...
    const track = new Track({
      ...
      filePath: cloudRes.secure_url, // Lưu URL Cloudinary
    ...
    res.status(201).json({
      message: 'Track uploaded & clouded successfully',
      track: {
        id: track._id,
        title: track.title,
        artist: track.artist,
